...
function buildUserPrompt(decision: ResponseDecision, locale: Locale): string {
  ...
  // Grounded Forward Intent Gate 2 — the structured intent is the sole
  // authority for WHAT is still legitimately open; this model only
  // decides HOW to phrase it (wording/shape/acknowledgement amount stay
  // fully its own choice, per MODE_RULES.ask above).
  if (decision.mode === "ask" && decision.groundedForwardIntent) {
    parts.push(
      `the ONLY legitimate open dimension you may ask about: ${decision.groundedForwardIntent.openDimension.description}. Do not ask about anything else — do not invent a different gap, do not analyze a relationship. Stay at exactly the scope that sentence states: if it itself names a reason/basis/cause as the open thing, you may ask for that; if it does not (e.g. it only says a further facet, surrounding context, what happened, or an unspecified elaboration is open), keep your question in neutral WHAT/WHAT-ELSE territory — do not narrow it into WHY, WHAT CAUSED, WHAT MADE THIS HAPPEN, or any other phrasing that presumes a specific trigger exists.`
    );
  }
